...
######################### Running analysis with first relative differences
Data1              <- data.table(subset(Eastern_HMDL, Age==0))
Data2              <- data.table(edagger)

Years              <-    Data1[, Y(Year = Year,lag.2 = 1), by = list(PopName,Sex)]
dif.e0             <-    Data1[, relative.change(x = ex), by = list(PopName,Sex)]
dif.ed             <-    Data2[, relative.change(x = edagger), by = list(PopName,Sex)]
...
Fig.stag <-     useOuterStrips(xyplot(rel.e0 ~ rel.ed|Period+Sex1,
                                      ...
                                      #layout=c(3,1),
                                      #between=list(x=1.5),
                                      key = list(x=.07,y=.81, title=NULL,background="transparent", 
                                                 text=list(c("Russia"))
                                                 ,cex=1,
                                      ...
                                      scales=list(x=list(at=c(-.1,0,-.05,.05,.1),cex=1.5),
                                                  y=list(cex=1.5)),
                                      panel = function(x, y, ...){   
                                        panel.abline(v=c(0),col='black',lty=2)         
                                        panel.abline(h=c(0),col='black',lty=2)         
                                        panel.text(-.05,-.05,labels=Prop.fig.T[panel.number()],cex=1.2)
                                        panel.text(-.05,-.06,labels=Prop.fig.TCI[panel.number()],cex=1.2)
                                        panel.text(.05,.07,labels=Prop.fig.1T[panel.number()],cex=1.2)
                                        panel.text(.05,.06,labels=Prop.fig.1TCI[panel.number()],cex=1.2)
                                        panel.xyplot(x, y, ...)                     
                                      }),strip.left = T)
Fig.stag
...
pdf(file="Latex/F5.pdf",width=13,height=9,pointsize=6)
print(Fig.stag2)
dev.off()

...
require(gridExtra)
pdf(file="R/Figures/SM_ex_males_85.pdf",width=16,height=7,useDingbats = F)
grid.arrange(ex.males1,ex.males2,ncol=2)
dev.off()



# Figures for lifespan variation------------------------------------
...
require(gridExtra)
pdf(file="R/Figures/SM_ed_males_85.pdf",width=16,height=7,useDingbats = F)
...
require(gridExtra)
pdf(file="R/Figures/SM_ex_females_85.pdf",width=16,height=7,useDingbats = F)
...
require(gridExtra)
pdf(file="R/Figures/SM_ed_females_85.pdf",width=16,height=7,useDingbats = F)
grid.arrange(ed.females1,ed.females2,ncol=2)
dev.off()

###Table summarising the results by cause of death
ex1        <- getData.function.table(Data2 = DT.Decomp.ex,state = 'National',initial = 1995,final = 2005)
ex1$Period <- paste0(1995,'-',2005)
ex2        <- getData.function.table(Data2 = DT.Decomp.ex,state = 'National',initial = 2005,final = 2015)
ex2$Period <- paste0(2005,'-',2015)
ex1$Ind    <- 'Life expectancy'
ex2$Ind    <- 'Life expectancy'

ed1        <- getData.function.table(Data2 = DT.Decomp.ed,state = 'National',initial = 1995,final = 2005)
ed1$Period <- paste0(1995,'-',2005)
ed2        <- getData.function.table(Data2 = DT.Decomp.ed,state = 'National',initial = 2005,final = 2015)
ed2$Period <- paste0(2005,'-',2015)
ed1$Ind    <- 'Lifespan inequality'
ed2$Ind    <- 'Lifespan inequality'

Table.data <- data.table(rbind(ex1,ex2,ed1,ed2))

Table1     <- Table.data[,sum(Contribution), by = list(Name,Sex,Cause,Period,Ind)]
Table1     <- Table1[order(Sex,Ind,Period,Cause)]
write.csv(Table1,'Manuscript/AJPH Submission/RR Submission/TableData1.csv')

DT.LTmx[DT.LTmx$state.name == 'National' & DT.LTmx$year %in% c(1995,2005) & DT.LTmx$age %in% 15,]

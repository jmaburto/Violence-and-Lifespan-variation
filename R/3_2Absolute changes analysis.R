...
dif.ed             <-    Data2[, absolute.change(x = edagger), by = list(PopName,Sex)]
...
Fig.stag <-     useOuterStrips(xyplot(rel.e0 ~ rel.ed|Period+Sex1,
                         ...
                         panel = function(x, y, ...){   
                           panel.abline(v=c(0),col='black',lty=2)         
                           panel.abline(h=c(0),col='black',lty=2)         
                           panel.text(-1.6,-2,labels=Prop.fig.T[panel.number()],cex=1.2)
                           panel.text(-1.6,-2.5,labels=Prop.fig.TCI[panel.number()],cex=1.2)
                           panel.text(1.65,2.5,labels=Prop.fig.1T[panel.number()],cex=1.2)
                           panel.text(1.65,2,labels=Prop.fig.1TCI[panel.number()],cex=1.2)
                           panel.xyplot(x, y, ...)                     
                         }),strip.left = T)
Fig.stag
...
pdf(file="Latex/F5Abs.pdf",width=13,height=9,pointsize=6)
print(Fig.stag2)
dev.off()

...
source('R/Version with INEGI data/2_1_LifeExpectancy&LifeDisparity.R')
...
states <- sort(unique(Dxs.cast$state))
sexes  <- sort(unique(Dxs.cast$sex))
years  <- sort(unique(Dxs.cast$year))
ages   <- sort(unique(Dxs.cast$age))
causes <- colnames(Dxs.cast)[5:20]
...
Sex.List
},LTC2 = Dxs.cast, years=years, Empty = Empty, causes = causes, ages=ages)
  ...
  for (j in states){
    ...
    Decomp.list <- mclapply(years[-length(years)],function(yr,y,e0frommxc,i){
      ...
      contrib           <- mydecomp(func = e0frommxc,
                          ...
                          rates2 = c(y[[as.character(yr+1)]]),N = 25,sex = i)
...
save(Decomp.results.list, file =  'Data/DecompResults_ex_List_INEGI_ill.RData')
gc()
...
save(Decomp.results.edagger, file =  'Data/DecompResults_ed_List_INEGI_ill.RData')
gc()
